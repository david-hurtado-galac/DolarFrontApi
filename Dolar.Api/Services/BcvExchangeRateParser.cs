...
public class BcvExchangeRateParser
{
    ...
    private static readonly Regex CnyCurrencyContextRegex = new(
       @"(?i)(?:CNY|RMB|YUAN|Yuan|Renminbi)[^<]{0,120}?(?<value>\d{1,3}(?:[.,]\d{1,8})+)",
       RegexOptions.CultureInvariant);
    ...
    public decimal? ParseCnyFromHtml(string html) {
        if (string.IsNullOrWhiteSpace(html)) {
            return null;
        }
        ...
        var contextMatch = CnyCurrencyContextRegex.Match(normalized);
        if (contextMatch.Success) {
            return ParseCandidate(contextMatch.Groups["value"].Value);
        }
